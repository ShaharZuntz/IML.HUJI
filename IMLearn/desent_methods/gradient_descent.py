...
class GradientDescent:
    ...
    callback_: Callable[[GradientDescent, ...], None]
        ...
        Callable function should receive as input a GradientDescent instance,
        and any additional arguments specified in the `GradientDescent.fit`
        function
    """
    ...
    def __init__(self,
        ...
        default=default_callback
            ...
            Callable function should receive as input a GradientDescent
            instance, and any additional arguments specified in the
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        n_samples = np.size(y)
        n_feature = np.size(X) // n_samples

        # initialize w_0
        w_t = np.copy(f.weights)

        # calculate objective value and gradient at w_0
        val = f.compute_output(X=X, y=y)
        grad = f.compute_jacobian(X=X, y=y)

        # initialize best w to w_0
        best_val, best_w = np.copy(val), np.copy(w_t)

        # initialize average w to w_0
        ave_w = np.copy(w_t)

        self.callback_(
            self, solver=self, weight=np.copy(w_t), val=np.copy(val),
            grad=np.copy(grad), t=0, eta=self.learning_rate_.lr_step(t=0),
            delta=0
        )

        # loop another self.max_iter_ times
        for t in range(1, self.max_iter_):
            old_w_t = np.copy(w_t)

            # calculate current learning rate (base on w_(t-1)
            eta = self.learning_rate_.lr_step(t=t)

            # calculate w_t
            w_t -= eta * grad

            # calculate Euclidean distance between w_t and w_(t-1)
            delta = np.linalg.norm(w_t - old_w_t)

            # update objective module
            f.weights = np.copy(w_t)

            # calculate objective value and gradient at w_t
            val = f.compute_output(X=X, y=y)
            grad = f.compute_jacobian(X=X, y=y)

            # update average w
            ave_w = (t * ave_w + w_t) / (t + 1)

            # update best w if necessary
            if val < best_val:
                best_val, best_w = np.copy(val), np.copy(w_t)
        ...
        return {"last": w_t, "average": ave_w, "best": best_w}[self.out_type_]
