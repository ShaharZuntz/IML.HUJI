...
class GradientDescent:
    ...
    callback_: Callable[[...], None], default=default_callback
        ...
        fitting to given data. Callable function receives as input any
        argument relevant for the current GD iteration. Arguments are
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        passing self and the following named arguments:
            ...
            - grad:  ndarray of shape specified by module's compute_jacobian
                function Module's jacobian with respect to the weights and
                at current point, over given data X,y
            - t: int
        ...
        self.callback_(**{
            "solver": self, "weight": np.copy(w_t), "val": np.copy(val),
            "grad": np.copy(grad), "t": 0,
            "eta": self.learning_rate_.lr_step(t=0), "delta": 0
        })

        # loop another self.max_iter_-1 times
